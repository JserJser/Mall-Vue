...
export const loadIndexSections = ({commit}) => {
  return new Promise((resolve, reject) => {
    // postData = qs.stringify(postData);
    Axios.get('http://192.144.186.149:8084/api/index/section')
      .then(function (response) {
        console.log(response);
        if (response.data.code === 200) {
          commit('SET_INDEXSECTION_INFO', response.data.data);
          resolve(true);
          return true;
        } else {
          resolve(false);
...
export const addSignUpUser = ({commit}, data) => {
  return new Promise((resolve, reject) => {
    ...
    postData.nickname = data.username;
    postData.password = data.password;
    postData.apassword = data.password;
    postData.profile = '';
    postData = qs.stringify(postData);
